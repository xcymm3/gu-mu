...
4. 选项只能通过 `effect` 修改状态：
   ...
   - `route`：仅在第二幕节点 7 锁定赵黎、纪清寒、苏莹、乔无咎四条路线；
...
第三至第五幕采用四组完全独立的固定场景：赵黎线表现追逐力量，纪清寒线表现主动关怀，苏莹线表现洞察真相，乔无咎线表现冷酷权谋。每组必须拥有 4／6／2 个线性节点，并固定保留两处剧情二选一，分别位于第三幕和第四幕；同一节点的两个选项必须拥有不同回应、相同去向，且不得附加状态效果或改变主结局。薛逢只在乔无咎线中负责引出外围机关与牵机副印，不得恢复成第五条路线。
