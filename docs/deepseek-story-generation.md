...
使用完整场景生成：每次请求一个 1000–2000 字符的完整场景，响应必须是唯一的 JSON 对象。脚本会自动先读取 `stories/briefs/00-generation-guide.txt`，再拼接命令行传入的场景 brief。前者规定世界观、人物动态卡和文风锚点；后者只提供本场景的剧情节拍、分支事实与收束位置。脚本在本地仅按句末分页，确保每页为 250–500 字；任何思考标记、分析文字、Markdown、截断或额外字段都会被拒绝。
