...
function describeBattleTurn(before: GameState, after: GameState, action: GuAction): BattleFeedback {
  ...
  const bloodGuardDefeat = battle.intent.id === "lash"
    ? "血傀儡的锁链绕过最后一层护体蛊息，横扫在你的下盘。你失去平衡，摔倒在半圆阵内。"
    : battle.intent.id === "smash"
      ? "血傀儡的重拳击穿最后一层护体蛊息，余劲将你压倒在阵线边缘。你勉强撑起一只手臂，却没能重新站起。"
      : "聚在血傀儡脚边的血浪越过最后一层护体蛊息，将你推出原先守住的位置。待血水退回阵槽，你已倒在血傀儡身前。";
  ...
  const suYanDefeat = battle.intent.id === "mist"
    ? "血雾渗过最后一层护体蛊息，胸中呼吸随之滞住。你脚下失力，倒在石台前。"
    : battle.intent.id === "seal"
      ? "合拢的棺印锁住手脚，最后一股蛊息也被压回经脉。你挣动数次，仍无法从阵光中起身。"
      : battle.intent.id === "feast"
        ? "导血槽中的牵引越过防守，带走你最后一股气力。你退到棺沿，随即失去支撑。"
        : battle.intent.id === "blooddemon"
          ? "血魔蛊的口器穿过最后一层防守，体内蛊息随气血一同下沉。你尚未抓住近旁石沿，便已倒地。"
          : "苏衍借窄井血气稳住躯体，随即以棺印压散你最后一层蛊息。你倒在石台前，再无力起身。";
  ...
  const bloodGuardResponse = battle.intent.id === "lash"
    ? immune
      ? "粉烟渗入胸腔血核，牵引左腕的蛊息随之错乱。锁链刚刚离地便偏向一侧，只在石面刮出一串碎石。"
      : defended
        ? "横扫而来的锁链撞上护体蛊力，沿着外侧滑开。沉重余劲震得你脚下后移，却没能将你扫倒。"
        : "锁链贴着石面扫中小腿外侧。你踉跄一步才重新站稳，原本准备前进的距离也被迫退了回去。"
    : battle.intent.id === "smash"
      ? immune
        ? "血核内的蛊息短暂断流，抬到高处的右臂随之一滞。重拳偏过你站立之处，砸碎了近旁一块石砖。"
        : defended
          ? "重拳正面落在护体蛊力上。你借势屈膝卸去冲力，鞋底擦过石面，仍守住了原先的位置。"
          : "粗重右拳穿过仓促架起的防守，撞在你的肩侧。你被余劲带到阵线边缘，胸中气血一阵翻涌。"
      : immune
        ? "粉烟扰乱了血核连续鼓动的节奏，刚在傀儡脚边聚起的血浪失去牵引，重新散回阵槽。"
        : defended
          ? "血浪撞上护体蛊力后向两旁分开。你顶住第一股冲力，没有被血水推出原位。"
          : "血浪沿石面迎面冲来，将你向后推开数尺。冰冷血水没过膝间，震得体内真元一时难以接续。";
  const suYanResponse = battle.intent.id === "mist"
    ? immune
      ? "惑心蛊散出的粉烟扰乱了苏衍引雾的方位。血雾在中途分向两侧，没能压到你身前。"
      : defended
        ? "血雾撞上护体蛊力后停在身外。冷意沿防御边缘游走，却没能压住你的呼吸。"
        : "血雾从四周合拢，沉重冷意压住口鼻。你收紧气息退开一步，体内真元也随之滞涩。"
    : battle.intent.id === "seal"
      ? immune
        ? "苏衍引动棺印的次序被粉烟扰乱。左右阵光先后合空，从你身侧错开。"
        : defended
          ? "两道棺印压上护体蛊力。你稳住脚下，任由阵光从防御外侧滑回棺沿。"
          : "左右阵光扣住四肢片刻。你挣开束缚时已被迫退到石台边缘，气机也乱了一层。"
      : battle.intent.id === "feast"
        ? immune
          ? "惑心蛊使苏衍错判了导血槽的流向。数股逆流在棺前相撞，牵引随即中断。"
          : defended
            ? "你收束气血，以护体蛊力隔开脚下阵槽。牵引数次贴近经脉，都被挡回石面。"
            : "导血槽中的牵引攀上双腿，一线气血被拉向棺前。你挣开阵路后，胸中已是一阵发空。"
        : battle.intent.id === "rest"
          ? immune
            ? "惑心蛊扰乱了苏衍的吐纳节奏。窄井残线刚刚亮起便重新暗下，未能送回血气。"
            : defended
              ? "你守住黑棺前的位置，没有贸然逼近。苏衍趁隙从窄井残线中抽回一股血气，枯槁四肢恢复了少许。"
              : "苏衍按住棺沿，从窄井残线中抽回一股血气。暗红微光重新填入四肢，他先前消耗的力量也恢复了少许。"
          : immune
            ? "粉烟令苏衍驭使血魔蛊的指令慢了半拍。蛊虫没有扑出，沿新凿阵线退回棺外。"
            : defended
              ? "血魔蛊的口器撞上护体蛊力，未能触及气血；苏衍仍从蛊虫带回的血线上收回少许力量。"
              : "血魔蛊贴近身侧，口器带走一线气血后立即退回苏衍掌中。暗红血线随之反哺他的枯槁躯体。";
...
function buildBattleResultText(game: GameState, won: boolean): string {
  ...
  if (won) {
    ...
    if (enemyName === "血傀儡" && game.sceneId === "suBloodGuard") {
      return "最后一击贯入血傀儡胸骨中央，暗红血核从中裂开。双肩与右膝的阵纹依次熄灭，左腕锁链落在石地上；傀儡先是向前跪倒，随后伏在半圆阵内。\n\n血核裂缝中逸出一缕尚未被祖阵收回的无主真元。苏莹仍压着旧阵，确认它不与导血槽相连后，示意你将其引入经脉。真元运转一周，原本滞涩的几处气路逐一贯通，你能够容纳的真元也比先前多出四点。\n\n苏莹等到傀儡身上的血光完全熄灭，才松开阵点。新凿细线仍在远处轻颤，却已经无法借这具守墓旧物拦住棺前缺口。你们避开断裂阵纹，走到黑棺前。";
    }
    if (enemyName === "苏衍" && game.sceneId === "suMasterDuel") {
      return "你最后一次攻势截断苏衍背后通往窄井的细线。失去祖阵支撑后，他抬起的手停在半空，枯槁躯体中的暗红血光从四肢向胸口回缩。苏衍试图退回石台，人形凹腔却因血钥印记已经断开而没有回应。\n\n苏莹压住棺沿最后一处主纹。苏衍背后的细线依次熄灭，躯体失去支撑，倒在石台与黑棺之间。你等到他的呼吸与石殿阵光一同停止，才确认墓主已无法再起。\n\n苏莹没有松开阵路。苏衍死后，祖阵各处开始失去约束，远端新凿细线也在反向收紧。棺底石轴持续震动，碎石不断从井壁落下。";
    }
  ...
  const defeatText: Record<string, string> = {
    ...
    血傀儡: game.sceneId === "suBloodGuard"
      ? "血傀儡重新站到黑棺前，左腕锁链横在唯一的缺口上。你身下的半圆旧阵逐段亮起，胸腔血核再次从阵槽中抽取血气。\n\n苏莹仍试图压住阵点，新凿细线却从外围越过她的封阻，与傀儡脚下旧纹重新接合。守墓阵纹拖动倒在阵内的闯入者，将你送向黑棺前的导血槽。\n\n气血与真元沿阵纹流向殿心蛊茧。你数次试图催动本命蛊，指尖却再也聚不起蛊息。苏莹被重新闭合的阵线隔在外侧，无法越过血傀儡将你拉回。"
      : "血傀儡重新站到石阶中央，截断了所有退路。你身下的祭纹逐段亮起，刚刚被压住的血线也再次收紧。气血与真元沿阵纹流向中央蛊茧；你试图撑起身体，指尖却再也聚不起半点真元。",
    ...
    苏衍: "苏衍越过倒地的你，将手重新按入黑棺的人形凹腔。苏莹仍压着祖阵主路，却无法同时守住窄井与棺侧支纹。苏衍背后的细线逐一复明，半圆旧阵也越过棺沿重新接合。\n\n导血槽从你身下亮起，气血与真元被祖阵送入窄井。苏衍尚未完成的躯体在暗红阵光中逐渐充实；苏莹被闭合的阵路隔在另一侧，无法越过重新压下的五转蛊息。你的意识随最后一缕蛊息离体而断绝。",
  };
...
function GameSettings({ audioSettings, onAudioChange, onBack, onClearEndings, onThemeChange, onToggleReduceMotion, reduceMotion, themePreference }: { audioSettings: AudioSettings; onAudioChange: (settings: AudioSettings) => void; onBack: () => void; onClearEndings: () => void; onThemeChange: (theme: ThemePreference) => void; onToggleReduceMotion: () => void; reduceMotion: boolean; themePreference: ThemePreference }) {
  ...
  return <main className="game-shell settings-shell"><ViewArtwork assetKey="ui.settings" /><section className="game-frame settings-card" aria-labelledby="settings-title">
    ...
    <div className="settings-list"><div className="settings-note theme-setting"><strong>界面主题</strong><p>选择蛊墓在此设备上的明暗样式。</p><div aria-label="选择界面主题" className="theme-options" role="group">{(["system", "light", "dark"] as ThemePreference[]).map((theme) => <button aria-pressed={themePreference === theme} className="theme-option" key={theme} onClick={() => onThemeChange(theme)}>{theme === "system" ? "跟随系统" : theme === "light" ? "亮色" : "暗色"}</button>)}</div></div>
      ...
      <button aria-pressed={reduceMotion} className="settings-row" onClick={onToggleReduceMotion}><span><strong>减少动态</strong><small>减少剧情与按钮的动画效果</small></span><em>{reduceMotion ? "已开启" : "跟随系统"}</em></button>
