...
export const scenes: Record<string, Scene> = {
  gate: {
    ...
    text: "蛊市夜雨未歇。乔无咎在荒原墓门前展开半张墓图，许诺五转蛊修坐化墓中的遗物见者有份。赵黎倚石把玩血纹蛊虫，明明面白如少年，开口却自称老夫；纪寒衣抱剑立在雨中，不与任何人说话；薛逢见谁都笑，嘴里已先说了三遍散修互照应；苏莹垂头描摹墓门蛊纹，像在背诵一句不该被人听见的话。\n\n你按住腰间祖传旧玉。玉在掌下微微发烫。赵黎隔着雨幕看了你一眼，嘴角那点笑意并未散去。乔无咎一拱手，率先踏进墓门。",
    choices: [
  ...
  swarm: {
    ...
    text: "入墓不过百步，石壁缝隙便涌出万千噬魂蛊，如黑潮压满甬道。薛逢第一个后退，却被乔无咎一句“退路已封”钉在原地。赵黎负手而行，近身的蛊虫无声自焚，像被更暴烈的蛊力从内里撕碎。\n\n蛊潮将你与纪寒衣挤到侧壁。一条毒藤缠住你脚踝，她剑光掠过，藤断而裤脚只裂一线；同一刻蛊蝎扑向她后颈，你反手将其拍碎。她只看了你一眼，低声道：“跟紧。”",
    choices: [
      { id: "shield-ji", label: "替纪寒衣清开侧后的蛊蝎", next: "shadow", effect: { trust: { ji: 1 }, flag: "寒衣援手" } },
  ...
  shadow: {
    ...
    text: "蛊潮退去后，石壁深处却有血影随火折子慢慢移动。你腰间旧玉骤然发烫，灼得皮肤生疼。赵黎停下脚步，隔着众人望来：“那块玉，借老夫瞧瞧？”\n\n你没有应声。他也不逼，只笑道：“一会儿你若死了，玉归老夫，提前说好。”纪寒衣恰好并肩，声音极低：“这墓的机关有生门，不像防人进，倒像在等什么人。”",
    choices: [
      { id: "listen-ji", label: "记下纪寒衣的“等人”之说", next: "chamber", effect: { flag: "生门低语", trust: { ji: 1 } } },
  ...
  illusion: {
    ...
    text: "石殿弥漫着甜腥蛊香。纪寒衣刚说出“屏息”，迷魂阵便已发动。你看见她卸下剑与冷意，踉跄着靠近；她也被幻境中的你牵住。迷魂香催的不是欲念，而是人最不愿示人的心防。\n\n阵破时，你们猛然分开。纪寒衣耳根通红，攥剑柄的手骨节发白，薛逢想笑，剑鞘已抵住他喉咙。没有人点破那片刻靠近，只是之后的脚步都比先前沉默。",
    choices: [
      { id: "protect-ji", label: "替纪寒衣挡住薛逢的讥笑", next: "puppets", effect: { trust: { ji: 1 }, flag: "寒衣心防" } },
  ...
  fog: {
    ...
    choices: [
      ...
      { id: "take-ji", label: "抓住纪寒衣的手", next: "routeTrial", effect: { route: "ji", trust: { ji: 1 } } },
  ...
  routeTrial: {
    ...
    text: (state) => routeText(state, {
      ...
      ji: "你与纪寒衣在狭窄陷道中沉默推进。她替你硬接一记傀儡重拳，虎口崩血；你引爆蛊种堵住追兵。她看着碎石，沉默许久才道：“你这种打法，活不过四十岁。”",
  ...
  routeTruth: {
    ...
    text: (state) => routeText(state, {
      zhao: "岔道口横着苏莹的尸身。赵黎曾从她身侧出手，却不是救她，只嫌她挡路。再往前，你找到纪寒衣半截断剑；她在另一条道里一敌五傀儡，剑断人亡。赵黎没有嘲笑你收剑，只说少两个人分蛊也未必是坏事。",
      ji: "你与纪寒衣每一步都在互相补位。她不再提迷魂阵，你也不问她要救的至亲。血色石门前，苏莹被祭阵隔空抽尽血气，抓住你的旧玉只来得及说半句“那块玉……是……”。纪寒衣握住残剑，目光第一次不再像冰。",
      xue: "苏莹倒在三步外，薛逢说来不及救，却把能切断祭阵一角的活蛊线藏得更深。你在岔道救下重伤的纪寒衣，她没有道谢，只用复杂眼神看着你与薛逢。所有人都在算账，连活命也被算成了价码。",
      su: "苏莹在你背上断断续续说，师父找了一辈子墓主的后人。她看向你的旧玉时神色愈发笃定。傀儡忽从阴影刺来；若旧玉、纪寒衣的生门低语与苏莹识活符的线索都已留下，旧玉会先一步爆出血光，否则这一击将贯穿她。",
    }),
    choices: [
      { id: "keep-cold", label: "收起冰寒蛊简，继续前行", next: "routeCost", requires: { route: "zhao" }, effect: { flag: "冰寒蛊简" } },
      { id: "hold-ji", label: "扶住纪寒衣，替她稳住气血", next: "routeCost", requires: { route: "ji" }, effect: { trust: { ji: 1 } } },
  ...
  routeCost: {
    ...
    text: (state) => routeText(state, {
      ...
      ji: "纪寒衣以残剑拄地，血线已在她腕间游走。她说自己进墓本为续魂蛊材，对五转蛊毫无兴趣；随后又补了一句，若血流蛊醒来，她会先斩阵眼。你没有答话，只把她站不稳的肩扶正。",
  ...
  bloodRoom: {
    ...
    text: (state) => `石门合拢，四壁血纹齐亮，数道血线汇向中央血池。池中蛊卵缓缓裂开，**五转血流蛊**的威压灌满石室。乔无咎不现身，只通过活蛊线从石壁中说：“四人的血，一个人的命，正好。”\n\n${state.route === "ji" ? "纪寒衣以残剑拄地，血线正自她体内流向血池。" : state.route === "su" && state.flags.includes("苏莹存活") ? "苏莹站在你身侧，指尖那滴血仍在石面暗纹中发亮。" : state.route === "xue" ? "薛逢已开始盘算向谁跪下最值钱。" : "赵黎站在你身侧，却像一头终于等到猎物围成一圈的狼。"}`,
  ...
  awakening: {
    ...
    text: (state) => routeText(state, {
      ...
      ji: "纪寒衣看着你，只说“你选”。她以冰蚕剑插入血池阵眼，愿用三息冻结替你炸出一线生机。你知道这一剑下去，她与自己的蛊种都可能碎裂。",
      xue: "薛逢抢先跪下，向乔无咎说依约把你带来。乔无咎却淡淡答祭品名单上也有他。你藏在聚灵蛊中的追踪印记顺着活蛊线逆行，控制室的位置终于在识海中亮起。",
      su: state.flags.includes("苏莹存活") ? "苏莹的血滴入石面，暗门在血池下缓缓升起。黑石棺椁破水而出，墓主并未坐化，而是以血流蛊为饵等待祭品。乔无咎惊怒未尽，已被反向活蛊线拖向血池。" : "苏莹的尸身仍在门边。你看着她未写完的血字，明白再等下去，血祭会吞掉所有人。你割开手腕，决定以自己的血替换祭品。",
    }),
  ...
  finale: {
    ...
    text: (state) => routeText(state, {
      ...
      ji: "纪寒衣已将冰蚕剑钉进阵眼。她要你带她离开，不要带走蛊。你若引爆自己的蛊种，血流蛊会在将醒未醒之间化灰；代价是你们二人修为尽废。",
      xue: "逆向活蛊线已指向控制室。乔无咎操控被截断的一瞬，就是夺蛊的唯一机会。薛逢还在求饶，你可以让他活着记账，也可以把他交回血祭。",
      su: state.flags.includes("苏莹存活") ? "墓主从黑石棺中复苏，五转威压压得血池翻涌。赵黎、纪寒衣、薛逢与苏莹都还活着；这是唯一能让五人联手的时刻。" : "你纵身跃入血池，准备以全部血液与血流蛊共生。苏莹未能回来，但你能决定这只蛊醒来后是吞人，还是被人驾驭。",
    }),
    choices: [
      { id: "duel-zhao", label: "以冰寒秘术压蛊，与赵黎决战", next: "zhaoBattle", requires: { route: "zhao", flags: ["冰寒蛊简"] } },
      { id: "break-array", label: "与纪寒衣一同炸毁祭阵", next: "ending", requires: { route: "ji" }, effect: { ending: "severed" } },
      ...
      { id: "fight-master", label: "唤众人联手，先斩复苏墓主", next: "masterBattle", requires: { route: "su", flags: ["苏莹存活"] } },
    ],
  },
  masterBattle: {
    id: "masterBattle", act: 4, node: 3, chapter: "第四幕 · 血流蛊室 · 节点 3 / 3", title: "墓主诈死",
    text: "墓主抬手，乔无咎被活蛊线拖入血池。赵黎先出手，纪寒衣以断剑重铸阵纹，苏莹用血脉干扰墓室，薛逢被你硬拽去堵住退路。你以旧玉为引，让血流蛊暂时认主。五人第一次真正站在同一边。",
    battle: { enemyName: "五转墓主", enemyHealth: 28, victoryNext: "ending", defeatNext: "ending", victoryFlag: "墓主已灭", defeatFlag: "墓主吞尽血食" },
  },
  zhaoBattle: {
...
export const endings: Record<string, Ending> = {
  demon: { id: "demon", name: "夺蛊成魔", epitaph: "血浪吞人，唯你仍立。", text: "你借冰寒秘术与旧玉压住血流蛊，反手吞没赵黎。墓门外月色如血，怀中只余纪寒衣的断剑。你成为唯一活人，也成为蛊道里再无人敢直呼其名的血蛊魔君。" },
  severed: { id: "severed", name: "破蛊断脉", epitaph: "蛊碎了，人还在。", text: "纪寒衣冻结阵眼三息，你引爆蛊种，血流蛊与血祭一同化灰。你们修为尽废，却互相搀扶走出天亮的墓门。江湖失去两名蛊修，山野多了一间安静药铺。" },
  tyrant: { id: "tyrant", name: "血蛊枭雄", epitaph: "活蛊线断，旧账才刚开始。", text: "你借薛逢藏起的活蛊线反制控制室，乔无咎被祭阵反噬，血流蛊残缺认主。薛逢活着替你记账，纪寒衣带断剑离开。你未成魔，却也再不是从前那个散修。" },
  ...
  true: { id: "true", name: "血脉归位", epitaph: "五人出墓，天光未负。", text: "墓主败亡，乔无咎化为枯骨，血室崩塌。赵黎留下未竟之战，薛逢发誓改邪，纪寒衣将寒蚕丝系在你腕上；苏莹红着眼问你以后会不会丢下她。你说，不丢了。" },
  death: { id: "death", name: "命丧血池", epitaph: "血流蛊醒，先吞活人。", text: "你在蛊室中失去最后一点真元。血祭没有停下，墓主与乔无咎的谋算都沉入血池，只剩血流蛊记得你的气息。" },
...
const patterns: Record<string, EnemyAction[]> = {
  ...
  "五转墓主": [{ id: "mist", damage: 4, cue: "墓主抬手时，血池中升起一层沉重血雾，连呼吸都像被人攥住。" }, { id: "seal", damage: 6, cue: "黑石棺上的蛊印逐一亮起，整座墓室都在回应墓主的心跳。" }, { id: "feast", damage: 9, cue: "墓主张开五指，血池中的残魂齐齐尖啸，似要将所有活人的气血一口吞尽。" }, { id: "rest", damage: 0, heal: 5, cue: "墓主闭目吸纳血池余烬，散开的威压正在重新凝实。" }],
...
function finishBattle(state: GameState, battle: Battle, won: boolean, health: number) {
  ...
  const final = battle.enemyName === "五转墓主" ? (won ? "true" : "death") : battle.enemyName === "赵黎" ? (won ? "demon" : "death") : undefined;
