...
const traitorOathEvents: VisualNovelEvent[] = [
  { type: "background", asset: "background.control-room", transition: "fade" },
  { type: "narration", text: "乔无咎把薛逢留下的活蛊线缠上你的手腕，以双方蛊息立下短暂血誓。他不信你的忠心，你也不信他的许诺；这份盟约唯一可信之处，是你们都准备在血魔蛊醒后除掉对方。" },
...
const traitorControlRoomEvents: VisualNovelEvent[] = [
  { type: "background", asset: "background.control-room", transition: "fade" },
  { type: "narration", text: "控制室的石墙后排列着整座蛊墓的活蛊线。乔无咎向你展示每一道机关，也承认自己早已摸清墓中七成结构。你坐上薛逢原先的位置，第一次从执棋者的角度看见仍在雾中挣扎的三名同行者。" },
];
...
const traitorTrapJiEvents: VisualNovelEvent[] = [
  ...
  { type: "narration", text: "纪清寒循着你故意留下的求救信号赶来。她认出你的身影后没有迟疑，直到脚下阵纹亮起，才明白被救之人正是启动机关的人。你拉紧活蛊线，将她与断剑一同锁入献祭甬道。" },
...
export const traitorActThreeScenes: Record<string, Scene> = {
  ...
  traitorOath: {
    ...
    choices: [{ id: "traitor-take-lines", label: "接过整座蛊墓的活蛊线", next: "traitorControlRoom", result: "血誓落成，你坐上控制室里的第二把石椅。" }],
  },
};
