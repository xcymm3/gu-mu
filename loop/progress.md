...
- 将任务语义完成与 Git 交付拆分：标为 `done` 但工作区未提交、远端不可达或本地远端未同步时记录 `delivery-pending`，下一轮优先以 delivery 模式恢复，不再误判为不可恢复错误。
- 每次 50 分钟 Worker 明确保留最后 8 分钟用于状态、Conventional Commit 与推送；超时检查不再覆盖为致命交付失败。
- 连续监督器仅在当前分支、HEAD、脏文件清单与二进制差异指纹全部匹配检查点时允许自动携带差异恢复；未知改动仍停止。交付连续失败沿用六次有界阈值。
- 单轮监督器异常改为最多三次跨周期有界重试，避免一次预检、最终门禁或临时子进程错误直接终止整个目标。

## 2026-08-29 · GM2H-006 全流程回归与诚实交付

- 生产审计先复现 `next > postcss > nanoid` 的 `GHSA-2v37-7h3g-55p8` 高危阻断；在 `pnpm-workspace.yaml` 将受影响版本最小覆盖到 `nanoid@3.3.18`，未升级框架或改变游戏逻辑。
- `pnpm install --frozen-lockfile --prefer-offline` 退出码 0：451 个锁文件条目通过供应链策略；`pnpm why nanoid` 确认 Next.js 与 Tailwind 的两条 PostCSS 路径均解析为 `3.3.18`。
- 两次独立 `pnpm test:e2e:run` 均退出码 0：每次 11 项 Chromium 用例全部通过（5.3 分钟、3.5 分钟），四条正式路线、三种身份差异、九个结局、关键战斗胜败与手动／快速存读档未出现非确定性失败。
- 逐张目视复核本轮生成的 18 张 PNG，覆盖主菜单、身份选择、普通／多人对白、三选项、战斗、历史、设置、存档、结局、1280×720、1366×768、1920×1080、390×844 竖屏提示和 844×390 横屏舞台；未发现已知遮挡、裁切、溢出或风格阻断。
- 首次 `pnpm verify` 的快速门禁和生产构建通过，但浏览器启动曾因并发本地静态服务器短暂占用 `127.0.0.1:4173` 而退出码 1；占用进程自行退出且未发生测试断言失败，随后完整重跑不再复现。
- 最终 `pnpm verify` 退出码 0：92 项 Node 测试、ESLint、TypeScript、Next.js 16.3.0 生产静态构建与 11 项 Chromium 用例全部通过（浏览器阶段 3.5 分钟）；页面错误、`console.error`、失败资源请求和 HTTP 4xx/5xx 门禁均未触发。
- 最终 `pnpm audit --prod --registry=https://registry.npmjs.org` 退出码 0：`No known vulnerabilities found`；7/7 Loop 任务均完成，交付范围、证据和剩余风险记录于 `DONE.md`。
